...
const WardValidationSchema = {
  ...
  numberOfDutyCycles: Joi.number()
    .required()
    .min(1)
    .label('Number of duty cycles'),
  shifts: Joi.array()
    .items(objectId())
    .min(1)
    .unique()
    .label('Shifts/Duty cycles'),
  minNumberOfDoctors: Joi.number().min(0).label('Minimum number of doctors'),
  maxNumberOfLeaves: Joi.number()
    .min(0)
    .max(31)
    .label('Maximum number of leave dates'),
  minNumberOfDoctorsPerShift: Joi.number()
    .min(0)
    .label('Minimum number of doctors per shift'),
  allowAdjacentShifts: Joi.boolean()
    .required()
    .label(
      'Same doctor has not to be given both last shift of the day & first shift of the next Day'
    ),
